...
const OUT = customFile
  ? join(dirname(customFile), `${basename(customFile, extname(customFile))}-consistency.json`)
...
async function run() {
  console.log(`\nDecision Challenger — consistency test`)
  console.log(`Document: ${customFile ?? '(built-in sample)'}`)
  ...
  const output = {
    meta: { model: MODEL, stakes: STAKES, runs: RUNS, document_title: documentTitle },
